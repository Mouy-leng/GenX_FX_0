...
export default function DiscordBot() {
  ...
  const testBotMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest('/api/discord/test', {
        method: 'POST',
  ...
  const discordBot = botStatuses?.find((bot: any) => bot.botName === 'discord');
  const discordTransmissions = transmissions?.filter((t: any) => t.destination === 'discord') || [];
  const isOnline = healthData?.services?.discord;

  return (
    <div className="flex-1 overflow-auto">
      <Header
        title="Discord Bot"
        subtitle="Trading signal notifications and community integration"
      />
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                ...
                <Bot size={24} className={isOnline ? "text-green-500" : "text-red-500"} />
              </div>
              <p className="text-sm text-muted-foreground mt-2">
                {discordBot?.status || 'Unknown'}
              </p>
            </CardContent>
          </Card>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-500">
                    {discordTransmissions.length > 0 
                      ? `${Math.round((discordTransmissions.filter((t: any) => t.status === 'sent').length / discordTransmissions.length) * 100)}%`
                      : '0%'
                    }
                  </p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-400">
                    {discordBot?.lastHeartbeat 
                      ? new Date(discordBot.lastHeartbeat).toLocaleTimeString()
                      : 'Never'
                    }
                  </p>
                </div>
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    ...
                    <span className="text-sm">
                      {process.env.DISCORD_BOT_TOKEN ? 'Token configured' : 'Token missing'}
                    </span>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <div className={cn("w-2 h-2 rounded-full", process.env.DISCORD_CHANNEL_ID ? "bg-green-500" : "bg-yellow-500")} />
                    <span className="text-sm">
                      {process.env.DISCORD_CHANNEL_ID ? 'Channel configured' : 'Default channel'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  ...
                  <Badge variant="secondary">Market Updates</Badge>
                  <Badge variant="secondary">System Alerts</Badge>
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                ...
                <Textarea
                  id="testMessage"
                  placeholder="Enter test message to send to Discord..."
                  value={testMessage}
                  ...
                  {discordTransmissions.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-muted-foreground">
                        No Discord notifications sent yet
                      </td>
                    </tr>
                  ) : (
                    discordTransmissions.map((transmission: any) => {
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <h4 className="font-medium text-sm">Notification Format</h4>
                  <div className="mt-2 text-sm text-muted-foreground">
                    <p>• Rich embeds with signal details</p>
                    <p>• Color-coded by signal type</p>
                    <p>• Includes confidence levels</p>
                    <p>• Entry, target, and stop loss prices</p>
                  </div>
                </div>
              </div>
            </div>
